...
export const Home = (props) => {
    ...
    return (
        <ContainerHome>
            ...
            <Cards>
                {props.products.map(item => <ProductCard key={item.id} imageUrl={item.imageUrl[item.indexImage]} name={item.name} value={coinBrl(item.value)} addCart={props.addCart} idItem={item.id} />)}
            </Cards>
